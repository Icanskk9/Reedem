    ...
    try:
        ...
        if not verif_input:
            ...
            with open("logs/imei_gagal.txt", "a", encoding="utf-8") as f:
                f.write(f"{imei} | {EMAIL} | GAGAL: Input kode verifikasi tidak ditemukan\n")
            return
        ...
        try:
            ...
            if "Kupon" in hadiah_nama and "Rp 75.000" in hadiah_desc:
                ...
                with open("logs/imei_gagal.txt", "a", encoding="utf-8") as f:
                    f.write(f"{imei} | {EMAIL} | GAGAL: Hadiah tidak terdeteksi\n")
        except:
            print("❌ Tidak ada info hadiah ditemukan.")
            with open("logs/imei_gagal.txt", "a", encoding="utf-8") as f:
                f.write(f"{imei} | {EMAIL} | GAGAL: Tidak ada info hadiah ditemukan\n")

        time.sleep(1)
    except Exception as e:
        print(f"🚨 Error saat proses IMEI {imei}: {e}")
        with open("logs/imei_gagal.txt", "a", encoding="utf-8") as f:
            f.write(f"{imei} | {EMAIL} | ERROR: {e}\n")
    finally:
        driver.quit()
